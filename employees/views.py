...
def add_department(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('add_department')  # Redirect after saving
    else:
        form = DepartmentForm()
...
def add_designation(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('add_designation')  # Redirect after saving
    else:
        form = DesignationForm()

    designations = Designation.objects.all()
    return render(request, 'add_designation.html', {'form': form, 'designations': designations})
